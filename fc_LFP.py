...
def main(i):
    ...
    input_size = 1500 # this is the number of the input_data of the LSTM, i.e. the number of points used for forecasting
    ...
    output_size = 150 # this is th number of output_data of the LSTM, i.e. the future points forecasted by the LSTM
    ...
    save_load_model_number = 'None' # καθορίζει ποιο LSTM μοντέλο θα φορτωθεί (η αποθήκευση γίνεται στο φάκελο και τα μεταφέρεις manually στους φακέλους model)
    ...
    # scaler = signal_handler.lfp_scaler(scaling_method, scaling_power=4) # αυτό ήταν πριν αρχίσεις να χρησιμοποιήσει 2 scalers για να μη προκαλείται σφάλμα tensor*ndarray
    ...
    if remote_PC: file_name = 'LSTM_fc_data_All_ds1000.npy' # remote PC load file
    if remote_PC: save_load_path = PATH + 'project_files/' + file_name # remote PC load file
    ...
    # prepare data
    if (train_LSTM or train_older): train_loader, val_loader, tensor_scaler  = prepare_data(lfp_data, tensor_scaler, input_size, output_size, sliding_window_step, batch_size)
    ...
    if train_LSTM:
        ...
        lstm_model, training_string = training_lstm(lstm_model_init, criterion, optimizer, epochs, train_loader, val_loader, tensor_scaler, save_name='LSTM_forecasting_model', measure_train_time=True) 
    ...
    if train_LSTM:
        ...
        if generate_or_compare == 'generate': gen_signal = lstm_generate_lfp(lstm_model, test_signal[:starting_point], num_gen_points, tensor_scaler, only_gen_signal=1); plt.plot(gen_signal); plt.show(); plt.close()
        if generate_or_compare == 'compare':
            base_signal, gen_signal, = produce_comparing_signals(lstm_model, test_signal, starting_point, num_gen_points, tensor_scaler)
    ...
    if train_older:
        linear, dummy = train_older_methods(lfp_data, ndarray_scaler, input_size, output_size, sliding_window_step, batch_size, linear_save_name, dummy_save_name)
    if not(train_older):
    ...
    lstm_MAE_list, linear_MAE_list = produce_metric_samples(test_series, lstm_model, linear, number_of_st_points, num_gen_points, tensor_scaler, ndarray_scaler, metric ='MAE')
    ...
    lstm_MAE_list, dummy_MAE_list = produce_metric_samples(test_series, lstm_model, dummy, number_of_st_points, num_gen_points, tensor_scaler, ndarray_scaler, metric ='MAE')
    statistical_comparison(lstm_MAE_list, dummy_MAE_list, normality_test ='SW')
    print('\n\n\nCOMPARISON OF LSTM FORECASTING WITH PINK NOISE')
    lstm_MAE_list, noise_MAE_list = produce_metric_samples(test_series, lstm_model, 'pink_noise', number_of_st_points, num_gen_points, tensor_scaler, ndarray_scaler, metric ='MAE')
    ...
    lstm_gen_signal = lstm_generate_lfp(lstm_model, tensor_test_signal[:starting_point], num_gen_points, tensor_scaler, only_gen_signal=1)
...
### data preparation -> αυτή η συνάρτηση ήταν η αρχική σου αλλά την αντικατέστησες επειδή δημιουργούνταν προβλήματα μνήμης
...
def load_lstm(model_number, input_size, hidden_state_dim, num_layers, output_size):
    ...
    if model_number == 'None': model.load_state_dict(torch.load(PATH + 'project_files/LSTM_forecasting_model.pt'))
    else: model.load_state_dict(torch.load(PATH + 'project_files/models/model' + str(model_number) + '/LSTM_forecasting_model.pt'))
...
def save_params(downsample_scale, hidden_state_dim, num_layers, batch_size, lr, window_step, scaling_method, tag, input_size, output_size, training_string, i):
    ...
    if i == 'None':
        ...
        with open(PATH + '/project_files/LSTM_params' + str(i) + '.pkl', "wb") as file: pickle.dump(dict_param, file)
...
def load_params(model_number):
    ...
    if model_number == 'None':
        with open(PATH + 'project_files/LSTM_params.pkl', 'rb') as file: dict_param = pickle.load(file)
    else:
...
def train_older_methods(lfp_data, scaler, input_size, output_size, sliding_window_step, batch_size, linear_save_name:str, dummy_save_name:str):
...
def produce_metric_samples(test_series, lstm_model, comparing_model, number_of_st_points, num_gen_points, tensor_scaler, ndarray_scaler, metric):
    ...
    for starting_point in starting_points_list:
        ...
        # linear_gen_signal = ml_generate_lfp(linear, test_signal[:starting_point], input_size, output_size, num_gen_points, ndarray_scaler, only_generated = 1)
        # dummy_gen_signal = ml_generate_lfp(dummy, test_signal[:starting_point], input_size, output_size, num_gen_points, ndarray_scaler, only_generated = 1)
        if comparing_model not in ['white_noise', 'pink_noise', 'brownian_noise']:
            comparing_gen_signal = ml_generate_lfp(comparing_model, test_signal[:starting_point], input_size, output_size, num_gen_points, ndarray_scaler, only_generated = 1)
        ...
        if metric == 'MAE':
            ...
            metric_comparing = mean_absolute_error(actual_signal, comparing_gen_signal)
        elif metric == 'RMSE':
            metric_lstm = np.sqrt(mean_squared_error(actual_signal, lstm_gen_signal))
            metric_comparing = np.sqrt(mean_squared_error(actual_signal, comparing_gen_signal))
            ...
            metric_comparing, _ = stats.pearsonr(actual_signal, comparing_gen_signal)
            ...
            metric_comparing = np.max(sn.correlate(actual_signal, comparing_gen_signal)/(actual_signal.std()*comparing_gen_signal.std()*len(actual_signal)))
            ...
            _, Pxx_3 = sn.periodogram(comparing_gen_signal, fs=fs, return_onesided=True, scaling='density')
            metric_lstm = np.sqrt(mean_squared_error(Pxx_1, Pxx_2))
            metric_comparing = np.sqrt(mean_squared_error(Pxx_1, Pxx_3))

        lstm_metric_list.append(metric_lstm)
        comparing_metric_list.append(metric_comparing)
